...
class FeatureCombiner:
    ...
    def _collect_video_answers(
        ...
        placeholder = torch.zeros(video_shape)
        answers: Dict[str, torch.Tensor] = {}

        if all(int(label) == 1 for label in labels):
            return {f"Video_Answer{i}": placeholder.clone() for i in range(1, 6)}

        for idx, (step_value, _) in enumerate(zip(steps, labels), start=1):
            tensor = placeholder
            for key in self._video_lookup_keys(object_id, step_value):
                if key in video_lookup:
                    tensor = video_lookup[key].clone()
                    break
            answers[f"Video_Answer{idx}"] = tensor
        return answers

    def _video_lookup_keys(self, object_id: str, step_value) -> List[str]:
        keys: List[str] = []
        if step_value is None:
            return keys
        if isinstance(step_value, float) and math.isnan(step_value):
            return keys
        raw = str(step_value).strip()
        if not raw:
            return keys
        for alias in self._alias_step_tokens(raw):
            keys.append(f"{object_id}:{alias}")
        return keys
    ...
    def _load_pt_video_lookup(self, object_id: str, object_dir: Path) -> Tuple[Dict[str, torch.Tensor], torch.Size]:
        ...
        for pt_path in sorted(object_dir.glob("*.pt")):
            ...
            for alias in self._alias_step_tokens(step_id):
                lookup[f"{object_id}:{alias}"] = stacked
    ...
    def _alias_step_tokens(self, value: str) -> List[str]:
        aliases: List[str] = []
        if not value:
            return aliases
        base = value.strip()
        ...
        for d in digits:
            num = str(int(d))
            aliases.append(num)
            aliases.append(num.zfill(2))
            aliases.append(f"step{num}")
            aliases.append(f"step{num.zfill(2)}")
        if "_" in base:
            aliases.append(base.replace("_", ""))
