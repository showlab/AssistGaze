...
class EyeGazeQADataModule(pl.LightningDataModule):
    def __init__(self, batch_size: int = 1024):
        ...
        self.valid_set = dataset2


    def train_dataloader(self):
        return DataLoader(self.train_set, batch_size=self.batch_size,shuffle=True,num_workers=4,pin_memory=True,
                        #   collate_fn=EncodeData.collate_fn
                          )

    def val_dataloader(self):
        return DataLoader(self.valid_set, batch_size=self.batch_size,shuffle=True,num_workers=4,pin_memory=True,
                        #   collate_fn=EncodeData.collate_fn
                          )
    
    def test_dataloader(self):
        return DataLoader(self.valid_set, batch_size=self.batch_size,shuffle=False,num_workers=4,pin_memory=True,
                        #   collate_fn=EncodeData.collate_fn
                          )
